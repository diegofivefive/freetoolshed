...
import {
  ...
  parseTimestamp,
  formatDuration,
} from "@/lib/media-toolkit/video-trim";
import type { TrimResult } from "@/lib/media-toolkit/video-trim";
...
const GUIDE_SECTIONS: ToolGuideSection[] = [
  {
    ...
    content: "Drop a video file, enter time values, and click Trim or Split.",
    steps: [
  ...
  },
  {
    title: "Trim Mode",
    content:
      "Extracts a single clip between a start and end time. The output preserves original quality via stream copy — no re-encoding.",
  },
  {
    title: "Split Mode",
    content:
      "Cuts the video at one or more time markers, producing separate segments. For example, splitting at 1:00 and 3:00 produces three files: 0:00–1:00, 1:00–3:00, and 3:00–end.",
  },
  {
    title: "Time Formats",
    content:
  ...
  },
  {
    title: "Stream Copy",
    content:
      "This tool uses ffmpeg's -c copy mode, meaning cuts are nearly instant and preserve original video + audio quality. Cuts happen at the nearest keyframe, which may be slightly before your requested timestamp.",
  },
  {
    title: "Keyframe Note",
    content:
      "Video files have keyframes (I-frames) every 1–2 seconds. Stream copy cuts at the nearest preceding keyframe, so the actual cut point may be slightly earlier than requested. This is the tradeoff for instant, lossless trimming.",
  },
  {
    title: "Tips",
    content:
      "Use the video preview to scrub to the exact timestamp before cutting. For large files, processing is still fast because no re-encoding occurs. All processing happens in your browser — files never leave your device.",
  },
];
      ...
      <>
        ...
        >
          ...
          <h3 className="mt-4 text-lg font-semibold">
            Drop a video file here to trim or split
          </h3>
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-400">
          ...
          <span>
            Large video files take longer to load into memory. Stream copy
            ensures the actual cutting is fast, but reading and writing the
            file still depends on its size.
          </span>
        </div>
